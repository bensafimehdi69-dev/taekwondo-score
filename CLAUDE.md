...
- iPhone / iPad (26/09/2026) : le lecteur récent (PDF.js 5, build legacy) est utilisé partout en premier ; Safari n'avait pas l'itération `for await` des ReadableStream (utilisée par `getTextContent`), complétée dans `installPdfCompatibility`. L'ancien lecteur « pdfjs-ios » ne sert plus qu'en secours (ou `?pdfEngine=ios`) : ses contours de la police WT ne correspondent pas à `WT_GLYPH_MAP`, les pages partaient en OCR (GP de Rome : 8 vainqueurs de combat lus au lieu de 86). Vérifié dans le vrai Safari d'iPhone (simulateur iOS).
